...
- **Web tokens are the source of truth.** `packages/react/src/styles/globals.css` is the contract (`THEMING.md`, `packages/react/TOKEN_SPEC.md`); `packages/react-native` themes are generated from it (`generate-themes.mjs`, parity-tested). Never hand-diverge either side.
