...
- `src/components/`:
  ...
  - Data: `data-table/` — DataTable + toolbar, column-filters, column-header-filter, pagination-nav, `use-table-state.ts` (`useTableState`), chips, skeletons. Server-side patterns: `SERVER_SIDE_GUIDE.md`.
  - Charts (ApexCharts wrappers): `charts/` (chart, area-chart, bar-chart, line-chart, donut-chart, radial-bar-chart, chart-skeleton, theme-utils, `use-theme-aware-chart`), `sparkline-chart/`. `chart-skeleton` stays in `charts/` by design: all chart wrappers share it as their loading arm (single skeleton implementation, `charts.styles`-style sharing); `sparkline-chart/` is its own family because it renders a lighter-weight SVG sparkline, not an ApexCharts wrapper — it does not use chart-skeleton.
...
**Ported:** social-login-button (incl. merged SocialLoginButtons), footer, metric-card, session-card, user-menu-dropdown, app-shell, charts (Chart core + line/area/bar/donut/radial wrappers + chart-skeleton), dnd (SortableItem).
