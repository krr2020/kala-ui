...
- Sanctioned multi-component families (beyond `<name>.tsx` + `<name>-skeleton.tsx`): `skeleton/` ships `skeleton-wrapper.tsx` and `skeleton-patterns.tsx` (composition presets + a README) alongside `skeleton.tsx`; `error-boundary/` ships `error-fallback.tsx` (its default arm). No other family adds extra components without a note here.
- Component families (all 93 under `src/components/`; this inventory is exhaustive — keep it in lockstep with the directory):
  - Primitives/layout: box, flex, grid, stack, center, container, paper, card, group, list, separator, aspect-ratio, scroll-area, resizable.
  - Form: input, input-group, textarea, number-input, select, native-select, combobox, multi-select, checkbox, radio-group, switch, slider, rating, segmented-control, date-picker, time-picker, calendar, color-input, file-upload, input-otp, tag-input, password-strength-indicator, field, label.
...
- Config-table coverage is a port queue, not yet complete. Migrated families are exactly the tables in `src/config/` re-exported from `src/config/index.ts` — that barrel is the live list; ~35 of 93 families are ported. Unported families (mostly primitives/layout and Radix pass-throughs) still keep base strings and cva tables in their `.tsx` and MUST be ported migrate-on-touch: when you edit one, extract its table to `src/config/<name>.ts`, re-export from `src/config/index.ts`, and pin it in `src/__tests__/slot-styles.test.tsx` — mirroring react-app's `PORT_QUEUE` discipline. There is no standing exemption except pass-through families with no own base classes to extract.
