...
const MultiSelect = React.forwardRef<HTMLButtonElement, MultiSelectProps>(
  (
    ...
    const isIndeterminate =
      value.length > 0 && !isAllSelected && value.length < availableOptions.length;
              ...
              {selectedOptions.length === 0 ? (
                ...
                <>
                  {displayedOptions.map((option) => (
                    ...
                    >
                      {option.icon && (
                        <span className="mr-1 flex size-3 items-center">{option.icon}</span>
                      )}
                      {option.label}
                      <button
                        type="button"
                    ...
                    {noGroup.map((option, index) => {
                      ...
                      return (
                        <React.Fragment key={option.value}>
                          {showSeparators && index > 0 && <Separator className="my-1" />}
                          <CommandItem
                            value={option.label}
                            disabled={isDisabled}
