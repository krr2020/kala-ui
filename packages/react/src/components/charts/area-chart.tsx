...
export function AreaChart({
    ...
    const options: ApexOptions = {
      ...
      annotations: {
        yaxis: referenceLines.map((line) => ({
          y: line.value,
          borderColor: line.color || 'hsl(var(--destructive))',
          strokeDashArray: line.dashArray || 3,
          label: {
            text: line.label || '',
            style: {
              color: '#fff',
              background: line.color || 'hsl(var(--destructive))',
            },
          },
        })),
      },
  ...
  return (
    <div className={cn('bg-card rounded-lg p-4', className)}>
      <Chart options={chartOptions} series={series} type="area" height={height} width={width} />
    </div>
  );
}
