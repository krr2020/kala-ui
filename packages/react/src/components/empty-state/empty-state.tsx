...
function EmptyState({
  ...
  return (
    <div data-comp="empty-state" className={cn(emptyStateVariants({ variant, size }), className)} {...props}>
      <div
        className="flex h-20 w-20 items-center justify-center rounded-full bg-muted"
        aria-hidden="true"
      >
        {typeof resolvedIcon === 'string' ? (
          <span className={cn('inline-block h-10 w-10 text-muted-foreground', resolvedIcon)} />
        ) : IconComponent ? (
