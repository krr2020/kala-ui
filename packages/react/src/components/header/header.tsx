      ...
      >
        <div className="container mx-auto px-3 sm:px-4 lg:px-6 xl:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16 gap-2 sm:gap-4">
            {/* Logo */}
            <div className="shrink-0 min-w-0 max-w-[120px] sm:max-w-none">{logo}</div>
            ...
            >
              <div className={cn('flex', variant === 'default' ? 'mx-auto' : '')}>
                <NavigationMenu>
                  <NavigationMenuList>
                    {navLinks.map((link) => (
                      <NavigationMenuItem key={link.href}>
                        {link.children ? (
                          <>
                            <NavigationMenuTrigger
                              className={cn('text-sm lg:text-base', link.active && 'text-primary')}
                            >
                              {link.label}
                            </NavigationMenuTrigger>
                            <NavigationMenuContent>
                                    ...
                                    <NavigationMenuLink asChild>
                                      <a
                                        ...
                                        className={cn(
                                          ...
                                          child.active && 'text-primary bg-primary/10',
                                        )}
                                      >
                                        {child.label}
                                      </a>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 shrink-0">
              {/* Search Bar (Desktop) */}
              {searchBar && <div className="hidden lg:block w-48 xl:w-64">{searchBar}</div>}

              {/* Language Switcher (Desktop) */}
              {languageSwitcher && <div className="hidden lg:block">{languageSwitcher}</div>}

              {/* Theme Switcher (Desktop) */}
              {themeSwitcher && <div className="hidden lg:block">{themeSwitcher}</div>}

              {/* Notifications (Desktop) */}
              {notifications && <div className="hidden lg:block">{notifications}</div>}
                        ...
                        {(themeSwitcherMobile || languageSwitcherMobile) && (
                          <div className="p-4 space-y-6">
                            ...
                            <div className="flex flex-col gap-6">
                              {themeSwitcherMobile && (
                                <div className="w-full">{themeSwitcherMobile}</div>
                              )}
                              {languageSwitcherMobile && (
                                <div className="w-full">{languageSwitcherMobile}</div>
                              )}
                            </div>
                          </div>
                        )}
