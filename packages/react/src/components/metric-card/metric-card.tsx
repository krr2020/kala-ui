...
export interface MetricCardProps extends Omit<React.ComponentProps<'div'>, 'title'> {
  ...
  changeLabel?: string;
  subtitle?: string;
  variant?: 'primary' | 'success' | 'warning' | 'info' | 'destructive' | 'secondary' | 'default';
}

const MetricCard = React.forwardRef<HTMLDivElement, MetricCardProps>(
  (
    { title, value, icon, change, changeLabel, subtitle, className, variant = 'default', ...props },
    ref,
  ) => {
    ...
    const getChangeText = (changeValue: number) => {
      ...
      const defaultLabel =
        changeValue > 0 || changeValue < 0 ? `${absValue}% than last week` : 'No change';
    ...
    return (
      <Card ref={ref} data-comp="metric-card" className={cn(getVariantStyles(), className)} {...props}>
        <div className="p-6">
          ...
          {(change !== undefined || subtitle) && (
            <div className="text-sm">
              {change !== undefined ? (
                ...
                <div className={cn(isColorful ? 'opacity-80' : 'text-muted-foreground')}>
                  {subtitle}
                </div>
              ) : null}
            </div>
