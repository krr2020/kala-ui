...
function PaginationContent({
  ...
  return (
    <ul
      className={cn('flex flex-row items-center', spaced ? 'gap-2' : 'gap-1', className)}
...
function PaginationLink({
  ...
  const variantClasses = {
    ...
    outline: cn(
      'border border-input',
      'hover:bg-accent hover:text-accent-foreground',
      isActive && 'border-primary bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm',
    ),
    filled: cn(
...
function PaginationNext({ className, showLabel = true, children, ...props }: PaginationNextProps) {
  return (
    <PaginationLink
...
const PaginationVariantContext = React.createContext<PaginationContentProps['variant']>('default');

function PaginationContentWithContext({ variant = 'default', ...props }: PaginationContentProps) {
  return (
...
export function generatePagination(
  ...
  if (shouldShowRightEllipsis) {
    ...
    items.push({ type: 'page', page: totalPages - 1, key: `page-${totalPages - 1}` });
  }
