...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn('fixed inset-0 z-30 bg-black/50 backdrop-blur-sm', className)}
      {...props}
    />
  );
}
...
function DrawerHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn('grid gap-1.5 px-6 py-5 text-center sm:text-left', className)} {...props} />
  );
}

function DrawerFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn('mt-auto flex flex-col gap-2 border-t px-6 py-5', className)} {...props} />
  );
}
