...
export const DesignSystemOverview = () => {
	const renderComponentVariations = (
		...
		switch (componentName) {
			...
			case "Spinner":
				return [<Spinner key="default" />, <Spinner key="large" size="lg" />];
			case "Skeleton":
				return [
			...
			case "Card":
				return [
					...
					<Card key="image" className="w-full border-primary">
						...
						<CardContent>
							<p className="text-sm text-muted-foreground mb-3">
								Card with hero image and descriptive content.
							</p>
							<div className="flex gap-2">
					...
					<Card
						key="gradient"
						className="w-full bg-linear-to-br from-primary/10 to-primary/5 border-primary/30"
					>
							...
							<div className="flex items-start gap-4">
								<div className="w-16 h-16 rounded-lg bg-primary/20 flex items-center justify-center">
									<svg
										...
										stroke="currentColor"
										role="img"
										aria-labelledby="lightning-icon"
									>
								...
								<div className="flex-1">
									...
									<p className="text-sm text-muted-foreground">
										Elevated card style with gradient background and custom
										icon.
									</p>
								</div>
							</div>
			...
			case "Toggle":
				return [
					...
					<Toggle key="pressed" pressed>
						Pressed
					</Toggle>,
				];
			default:
				return [
					<span key="preview" className="text-sm text-muted-foreground">
						Preview
					</span>,
				];
		}
				...
				{designSystemComponents.map((component) => (
					...
					>
						<div className="space-y-4">
							{renderComponentVariations(component.name).map(
								(variation, idx) => (
