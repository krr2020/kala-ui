export const selectTriggerStyles = {
	base: "cursor-pointer bg-card text-foreground data-placeholder:text-muted-foreground aria-invalid:kala-ring-destructive/20 aria-invalid:border-destructive hover:bg-accent hover:text-accent-foreground flex w-fit items-center justify-between gap-2 rounded-[var(--kala-radius-input,var(--kala-radius-control))] border whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none disabled:cursor-not-allowed disabled:opacity-50 kala-surface-input kala-focus-ring *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
	variants: {
		size: {
