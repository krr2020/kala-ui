/**
 * Toast styles — sonner owns the per-toast DOM, so these bases feed its
 * `toastOptions.classNames` map; slotStyles.<part> merges over each entry
 * (see buildToastClassNames in components/toast/toast.tsx).
 */
...
export type ToastPart = keyof typeof toastStyles;
